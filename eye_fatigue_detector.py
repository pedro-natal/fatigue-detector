...
class EyeFatigueDetector:
    ...
    def __init__(self):
        print("👁️ Iniciando Detector Especializado em Olhos")
        print("🎯 Este detector usa APENAS análise de olhos!")
        ...
        if len(eyes) > 0:
            ...
            for i, (ex, ey, ew, eh) in enumerate(eyes[:2]):  # Máximo 2 olhos
                ...
                if np.any(features):
                    ...
                    cor = (0, 0, 255) if prob_sono > self.threshold_sono else (0, 255, 0)
                    cv2.rectangle(roi_color, (ex, ey), (ex+ew, ey+eh), cor, 2)
                    
                    # Mostra probabilidade no olho
                    cv2.putText(roi_color, f"{prob_sono:.2f}", 
                               (ex, ey-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, cor, 1)
    ...
    def executar(self):
        ...
        print("\n👁️ DETECTOR DE OLHOS INICIADO")
        print("🎯 Focando APENAS na análise de olhos!")
        ...
        try:
            ...
            while True:
                ...
                cv2.imshow('Detector Especializado em Olhos', frame_processado)
            ...
            if self.total_frames > 0:
                print(f"📈 Taxa de sono: {self.deteccoes_sono/self.total_frames*100:.1f}%")
